<?php
    $data = App\AudioCategory::query()
        ->whereHas('category_audios', function ($query) use ($channel_partner) {
            $query->where('audio.active', 1)->where('audio.user_id', $channel_partner->id)->where('audio.uploaded_by','Channel');
        })
        ->with([
            'category_audios' => function ($audios_videos) use ($channel_partner) {
                $audios_videos
                    ...
                    ->where('audio.active', 1)->where('audio.user_id', $channel_partner->id)->where('audio.uploaded_by','Channel')

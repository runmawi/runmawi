<?php
    ...
    $data = App\SeriesGenre::query()->whereHas('category_series', function ($query) use ($channel_partner) {})
        ->with([
            'category_series' => function ($series) use ($channel_partner) {
                $series->select('series.*')->where('series.active', 1)->where('series.user_id', $channel_partner->id)->where('series.uploaded_by','Channel')->latest('series.created_at');
            },
        ])
                ...
                <div class="row">
                    <div class="col-sm-12 ">
                        ...
                        <div class="iq-main-header d-flex align-items-center justify-content-between">
                            ...
                            <h4 class="main-title view-all"><a href="{{ route('SeriesCategory',[$series_genre->slug] )}}">{{ 'View all' }}</a></h4>
                        </div>

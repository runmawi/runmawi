<?php
    $comment_loop =  App\WebComment::where('source_id',$source_id)->where('commentable_type',$commentable_type)->whereNull('child_id')->get();
?>
...
<?php foreach( $comment_loop as $key => $comment ): ?>
    ...
    <div class="media-body">
            ...
            foreach ($reply_comment as $key => $reply_comments) : ?>
                ...
                <div>
                    <?php if( Auth::user() != null && Auth::user()->id != $reply_comments->user_id  && Auth::user()->role != 'register' ):?>
                        <button data-toggle="modal" data-target="#reply-modal-<?= $reply_comments->id ?>" class="btn btn-sm btn-link text-uppercase">Reply</button>
                    <?php endif; ?>
        ...
        <?php  include(public_path('themes/default/views/comments/comment_edit.blade.php')); ?>

        <?php  include(public_path('themes/default/views/comments/comment_reply.blade.php')); ?>

                    ...
                    <div id="subscribers_only" style="background: linear-gradient(180deg, rgba(0, 0, 0, 0.3), rgba(0, 0, 0, 1.3)), url('{{ url('/public/uploads/images/' . $episode->player_image) }}'); background-repeat: no-repeat; background-size: cover; height: 450px; padding-top: 150px;">
                        <div class="container-fluid">
                            <div class="col-sm-8 col-12 p-0">
                                <h4>{{ $episode->title }}</h4>
                                <p class="text-white mt-3">{{ html_entity_decode(strip_tags($episode->episode_description)) }}</p>
                            </div>
                            <div class="clear"></div>
                            ...
                            @if(Auth::check() && Auth::user()->role == "registered")
                                <div class="d-flex">
                                    <form method="get" action="{{ url('/becomesubscriber') }}">
                                        <button id="button" class="view-count rent-video btn text-white btn-primary mr-4">{{ __('Subscribe now') }}</button>
                                    </form>
                                    <button data-toggle="modal" data-target="#season-purchase-now-modal" class="view-count rent-video btn btn-primary">
                                        {{ __('Purchase Now') }}
                                    </button>
                                </div>
                            @elseif(Auth::check() && Auth::user()->role == "subscriber" )
                                <div class="d-flex">
                                    <button data-toggle="modal" data-target="#season-purchase-now-modal" class="view-count rent-video btn btn-primary">
                                        {{ __('Purchase Now') }}
                                    </button>

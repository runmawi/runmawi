                                      ...
                                      <?php if($episode_play_access == 0): ?>
                                              ...
                                              <?php if(Auth::guest() && $seasons->access == "ppv"): ?>
                                                  <div class="d-flex">
                                                      <form method="get" action="<?= URL::to('/becomesubscriber') ?>">
                                                          <button id="button" class="view-count rent-video btn btn-primary mr-4 text-white"><?php echo __(!is_null($button_text->subscribe_text) ? $button_text->subscribe_text : 'Subscribe Now'); ?></button>
                                                      </form>
                                                      <button data-toggle="modal" data-target="#season-purchase-now-modal-<?= $seasons->id; ?>" class="view-count rent-video btn btn-primary">
                                                          <?=  __(!is_null($button_text->purchase_text) ? ($button_text->purchase_text) : ' Purchase Now ') ?>
                                                      </button>
                                                  </div>
                                              <?php elseif(Auth::check() && Auth::user()->role == "registered" && $seasons->access == "ppv"): ?>
                                                  <div class="d-flex">
                                                      <form method="get" action="<?= URL::to('/becomesubscriber') ?>">
                                                          <button id="button" class="view-count rent-video btn text-white btn-primary mr-4"><?php echo __(!is_null($button_text->subscribe_text) ? $button_text->subscribe_text : 'Subscribe Now'); ?></button>
                                                      </form>
                                                      <button data-toggle="modal" data-target="#season-purchase-now-modal-<?= $seasons->id; ?>" class="view-count rent-video btn btn-primary">
                                                          <?=  __(!is_null($button_text->purchase_text) ? ($button_text->purchase_text) : ' Purchase Now ') ?>
                                                      </button>
                                          ...
                                          <?php elseif($series->access == "registered" && $seasons->access == "ppv"): ?>
                                              <?php if(Auth::check() && Auth::user()->role == "registered" && $seasons->access == "ppv"): ?>
                                                  <div class="d-flex">
                                                      <form method="get" action="<?= URL::to('/becomesubscriber') ?>">
                                                          <button id="button" class="view-count rent-video btn btn-primary text-white mr-4"><?php echo __(!is_null($button_text->subscribe_text) ? $button_text->subscribe_text : 'Subscribe Now'); ?></button>
                                                      </form>
          ...
          <!-- <div  style="background: url(<?=URL::to('/') . '/public/uploads/images/' . $series->image ?>); background-repeat: no-repeat; background-size: cover; height: 400px; margin-top: 20px;"> -->
            ...
            <div class="container">
              ...
              <form method="get" action="<?= URL::to('signup') ?>">
                  <button id="button" class="view-count rent-video btn btn-primary"><?php echo __(!is_null($button_text->subscribe_text) ? $button_text->subscribe_text : 'Subscribe Now'); ?></button>
              </form>
          <?php elseif($series->access == 'registered'): ?>
            <form method="get" action="<?= URL::to('signup') ?>">
                  <button id="button" class="view-count rent-video btn btn-primary">
                    <?php echo __(!is_null($button_text->registered_text) ? $button_text->registered_text : 'Register Now'); ?></button>
              </form>
          <?php elseif($series->ppv_status == 1): ?>
          <div class="d-flex">
            <form method="get" action="<?= URL::to('signup') ?>">
                  <button id="button" class="view-count rent-video btn btn-primary mr-4"><?php echo __(!is_null($button_text->subscribe_text) ? $button_text->subscribe_text : 'Subscribe Now'); ?></button>
              </form>

            <form method="get" action="<?= URL::to('signup') ?>">
                  <button id="button" class="view-count rent-video btn btn-primary">
                    <?php echo __(!is_null($button_text->purchase_text) ? ($button_text->purchase_text) : ' Purchase Now '); ?></button>
              </form>
          </div>
            <?php endif; ?></h2>
          ...
          <?php elseif(Auth::guest() && $series->ppv_status == 1  || Auth::guest() && $series->access == "registered" ):  ?>
              ...
              <form method="get" action="<?= URL::to('/signup') ?>">
                ...
                <?php elseif($series->ppv_status == 1 &&  $series->access == 'registered' ): ?>
                  ...
                  <div class="d-flex">
                  <form method="get" action="<?= URL::to('/signup') ?>">
                      <button id="button"  class="view-count rent-video btn btn-primary mr-4"><?php echo __(!is_null($button_text->registered_text) ? $button_text->registered_text : 'Register Now'); ?></button>
                  </form>
                  <form action="<?= URL::to('/signup') ?>">

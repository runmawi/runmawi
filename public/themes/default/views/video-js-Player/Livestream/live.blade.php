...
<style type="text/css">
    ...
    body.light .modal-content {
       background: {{ GetAdminLightBg() }} !important;
       color: {{ GetAdminLightText() }} !important;
    }
    ...
    .my-video.video-js .vjs-big-play-button span{ 
    ...
    #purchase-modal-dialog .modal-header.align-items-center {
       height: 70px;
       border: none;
    }
    
    #purchase-modal-dialog .modal-header.align-items-center .col-12 {
       height: 50px;
    }
    
    #purchase-modal-dialog .modal-header.align-items-center .d-flex.align-items-center.justify-content-end {
       height: 50px;
    }
    
    #purchase-modal-dialog .modal-header.align-items-center img {
       height: 100%;
       width: 100%;
    }
    
    .col-sm-7.col-12.details {
       border-radius: 10px;
       padding: 0 1.5rem;
    }
    
    div#video-purchase-now-modal {
    ...
    .d-flex.justify-content-between.title {
       border-bottom: 1px solid rgba(255, 255, 255, .5);
       padding: 10px 0;
    }
    
    .btn-primary-dark {
        background-color: rgba(var(--btn-primary-rgb), 0.8);
    }

    .btn-primary-light {
        background-color: rgba(var(--btn-primary-rgb), 0.3);
    }
    ...
    input[type="radio"].payment_btn,
    input[type="radio"].quality_option {
        appearance: none;
    ...
    input[type="radio"].payment_btn:checked,
    input[type="radio"].quality_option:checked {
        background-color: black;
        border-color: white;
    }

    input[type="radio"].payment_btn:checked::before, 
    input[type="radio"].quality_option:checked::before {
        content: '';
        width: 10px;
        height: 10px;
        border-radius: 50%;
...
<script src="{{ URL::to('node_modules/videojs-settings-menu/dist/videojs-settings-menu.js') }}" rel="stylesheet"></script>
<script src="{{ asset('assets/js/video-js/end-card.js') }}"></script>
<script src="https://www.paypal.com/sdk/js?client-id={{ $paypal_signature }}"></script>
                            ...
                            <div class="bc-icons-2">
                                <ol class="breadcrumb">
                                    <li class="breadcrumb-item">
                                        <a class="black-text" href="{{ route('liveList') }}">{{ ucwords(__('Livestreams')) }}</a>
                                    ...
                                    <li class="breadcrumb-item">
                                        <a class="black-text">{{ strlen($video->title) > 50 ? ucwords(substr($video->title, 0, 120) . '...') : ucwords($video->title) }}</a>
                                    </li>
                                </ol>
                            </div>
                        </div>
                            ...
                            <div class="col-sm-7 col-12 details">
                                ...
                                <div class="becomesubs-page">
                                    <div class="Stripe_button row mt-3 justify-content-around">  
                                        <div class="Stripe_button col-md-6 col-6 btn">
                                            <button class="btn text-white stripe-continue-btn" data-live-id="{{ $Livestream_details->id }}" data-price="{{ $Livestream_details->ppv_price }}" style="cursor: pointer; z-index: 1000; position: relative; width: 100%; height: 100%; display: block; padding: 10px 15px; border: none; background-color: #007bff; border-radius: 4px;">
                                                {{ __('Continue') }}
                                            </button>
                                        </div>
                                        <div class="Stripe_button col-md-5 col-5 btn">
                                            <button type="button" class="btn text-white" data-dismiss="modal" aria-label="Close" style="cursor: pointer; width: 100%; height: 100%; display: block; padding: 10px 15px; border: none; background-color: #6c757d; border-radius: 4px;">
                                                {{ 'Cancel' }}
                                            </button>
                                        </div>
                                    </div>
                                    ...
                                    <div class="row mt-3 justify-content-around"> 
                                        <div class="Razorpay_button col-md-6 col-6 btn">
                                            @if ($Razorpay_payment_setting && $Razorpay_payment_setting->payment_type == 'Razorpay')
                                                <button class="btn text-white razorpay-continue-btn" data-live-id="{{ $Livestream_details->id }}" style="cursor: pointer; z-index: 1000; position: relative; width: 100%; height: 100%; display: block; padding: 10px 15px; border: none; background-color: #007bff; border-radius: 4px;">
                                                    {{ __('Continue') }}
                                                </button>
                                            @endif
                                        </div>
                                    ...
                                    <div class="row mt-3 justify-content-around">
                                        @if ($paypal_payment_setting && $paypal_payment_setting->payment_type == 'PayPal')
                                            <div class="paypal_button col-md-6 col-6 btn text-white paypal_pay_now" type="button" id="paypal_pay_now" onclick="paypal_checkout({{ $Livestream_details->id }}, {{ $Livestream_details->ppv_price }})">
                                                {{ "Continue" }}
                                            </div>
                                        @endif
...
<script type="text/javascript">
    $(document).ready(function() {
        ...
        $(".payment_btn").click(function() {
            ...
            let payment_gateway = $('input[name="payment_method"]:checked').data('value');
        ...
        // Razorpay Continue button handler
        $(document).on('click', '.razorpay-continue-btn', function(e) {
            e.preventDefault();
            
            const liveId = $(this).data('live-id');
            
            if (!liveId) {
                alert('Error: Missing live ID');
                return;
            }
            
            const razorpayUrl = '{{ route('RazorpayLiveRent', ['live_id' => 'PLACEHOLDER']) }}'.replace('PLACEHOLDER', liveId);
            
            if (!razorpayUrl || razorpayUrl.includes('PLACEHOLDER')) {
                alert('Error: Invalid URL generated');
                return;
            }
            ...
            try {
                ...
                alert('Error: Could not redirect to payment page. Please try again.');
            }
        });

        // Stripe Continue button handler
        $(document).on('click', '.stripe-continue-btn', function() {
            const liveId = $(this).data('live-id');
            const price = $(this).data('price');

            const stripeUrl = '{{ route('Stripe_payment_live_PPV_Purchase', ['live_id' => 'PLACEHOLDER_LIVE_ID', 'amount' => 'PLACEHOLDER_AMOUNT']) }}'
                .replace('PLACEHOLDER_LIVE_ID', liveId)
                .replace('PLACEHOLDER_AMOUNT', price);
...
<script>
    ...
    var user_name = '{{ Auth::check() ? Auth::User()->username : "" }}';
    var email = '{{ Auth::check() ? Auth::User()->email : "" }}';
    var mobile = '{{ Auth::check() ? Auth::User()->mobile : "" }}';

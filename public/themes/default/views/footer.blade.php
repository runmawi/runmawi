...
<script>
  document.addEventListener("DOMContentLoaded", function() {var lazyloadImages = document.querySelectorAll("img.lazy");var lazyloadThrottleTimeout;function lazyload () {if(lazyloadThrottleTimeout) {clearTimeout(lazyloadThrottleTimeout);}lazyloadThrottleTimeout = setTimeout(function() {var scrollTop = document.documentElement.scrollTop || document.body.scrollTop;lazyloadImages.forEach(function(img) {if (img.offsetTop < (document.documentElement.clientHeight + scrollTop)) {img.src = img.dataset.src;img.classList.remove('lazy');}});if (lazyloadImages.length == 0) {document.removeEventListener("scroll", lazyload);document.body.removeEventListener("resize", lazyload);window.matchMedia("(orientation: portrait)").removeListener(lazyload);}}, 20);}document.addEventListener("scroll", lazyload);document.body.addEventListener("resize", lazyload);window.matchMedia("(orientation: portrait)").addListener(lazyload);});
</script>
<?php } ?>

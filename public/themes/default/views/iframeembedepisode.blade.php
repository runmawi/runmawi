...
if($episode->type == 'm3u8'){
  $episodeURL =   URL::to('/storage/app/public/').'/'.$episode->path . '.m3u8'  ;
  $episode_player_type =  'application/x-mpegURL' ;
}elseif ($episode->type == 'file' || $episode->type == 'upload'){
  $episodeURL =   $episode->mp4_url  ;
  $episode_player_type =  'video/mp4' ;
}elseif ($episode->type == 'm3u8_url'){
  $episodeURL =   $episode->url  ;
  $episode_player_type =  'application/x-mpegURL' ;
}elseif($episode->type == 'bunny_cdn'){
 $episodeURL =   $episode->url  ;
 $episode_player_type =  'application/x-mpegURL' ;
}else{
  $episodeURL =  null ;
...
?>
          ...
          <video id="my-video" class="vjs-theme-city my-video video-js vjs-big-play-centered vjs-play-control vjs-fluid vjs_video_1462 vjs-controls-enabled vjs-picture-in-picture-control vjs-workinghover vjs-v7 vjs-quality-selector vjs-has-started vjs-paused vjs-layout-x-large vjs-user-inactive" controls
                width="auto" height="auto" poster="<?= URL::to('public/uploads/images/'.$episode->player_image) ?>" playsinline="playsinline"
                >
...
var videoId = "<?php echo $episode->id; ?>";
var PPV_Plan = "";

...
@forelse ($order_settings as $key => $item)
    ...
    @if(  $item->video_name == 'artist' && $home_settings->artist == 1 )        {{-- Artist --}}
        {!! Theme::uses($current_theme)->load("public/themes/{$current_theme}/views/partials/home/artist-videos",  array_merge($homepage_array_data) )->content() !!}
    @endif
    ...
    @if(  $item->video_name == 'albums' && $home_settings->albums == 1 )        {{-- Albums --}}
        {!! Theme::uses($current_theme)->load("public/themes/{$current_theme}/views/partials/home/latest-albums", array_merge($homepage_array_data, ['data' => $albums]) )->content() !!}
    @endif

    @if( $item->video_name == 'category_videos' && $home_settings->category_videos == 1 ) {{-- Videos Based on Category  --}}
        {!! Theme::uses($current_theme)->load("public/themes/{$current_theme}/views/partials/home/videos-based-categories", array_merge($homepage_array_data,['order_settings_list' => $order_settings_list ]) )->content() !!}
    @endif
    ...
    {!! Theme::uses($current_theme)->load("public/themes/{$current_theme}/views/partials/home/ContentPartners", array_merge($homepage_array_data, ['data' => $ModeratorsUser]) )->content() !!}
    @endif

    @if(  $item->video_name == 'latest_viewed_Livestream' && $home_settings->latest_viewed_Livestream == 1 ) {{-- Latest Viewed Livestream    --}}
        {!! Theme::uses($current_theme)->load("public/themes/{$current_theme}/views/partials/home/latest_viewed_Livestream", ($homepage_array_data) )->content() !!}
    @endif

    @if(  $item->video_name == 'latest_viewed_Episode' && $home_settings->latest_viewed_Episode == 1 ) {{-- Latest Viewed Episode   --}}
        {!! Theme::uses($current_theme)->load("public/themes/{$current_theme}/views/partials/home/latest_viewed_Episode", ($homepage_array_data) )->content() !!}
    @endif
    ...
    @if(  $item->video_name == 'Audio_Genre_audios' && $home_settings->AudioGenre_audios == 1 ) {{-- Audios Based on Category  --}}
        {!! Theme::uses($current_theme)->load("public/themes/{$current_theme}/views/partials/home/Audios-based-categories",  ($homepage_array_data) )->content() !!}
    @endif

    @if(  $item->video_name == 'live_category' && $home_settings->live_category == 1 ) {{-- LiveStream Based on Category  --}}
        {!! Theme::uses($current_theme)->load("public/themes/{$current_theme}/views/partials/home/livestreams-based-categories", ($homepage_array_data) )->content() !!}
    @endif

    @if(  $item->video_name == 'liveCategories' && $home_settings->liveCategories == 1 )       {{-- Live Categories --}} 
        {!! Theme::uses($current_theme)->load("public/themes/{$current_theme}/views/partials/home/liveCategories", array_merge($homepage_array_data, ['data' => $LiveCategory]) )->content() !!}
    @endif
    ...
    @if(  $item->video_name == 'ChannelPartner' && $home_settings->channel_partner == 1 )        {{-- channel partner  --}}
        ...
        {!! Theme::uses($current_theme)->load("public/themes/{$current_theme}/views/partials/home/ChannelPartners", array_merge($homepage_array_data, ['data' => $channels]) )->content() !!}
    @endif

    @if(  $item->video_name == 'latest_viewed_Videos' && $home_settings->latest_viewed_Videos == 1 ) {{-- Latest Viewed Videos --}}
        {!! Theme::uses($current_theme)->load("public/themes/{$current_theme}/views/partials/home/latest_viewed_Videos", ($homepage_array_data) )->content() !!}
    @endif

    @if(  $item->video_name == 'latest_viewed_Audios' && $home_settings->latest_viewed_Audios == 1 ) {{-- Latest Viewed Audios    --}}
        {!! Theme::uses($current_theme)->load("public/themes/{$current_theme}/views/partials/home/latest_viewed_Audios", ($homepage_array_data) )->content() !!}
    @endif
    ...
    @if(  $item->video_name == 'Audio_Genre' && $home_settings->AudioGenre == 1 )        {{-- Audio Genre  --}}
        {!! Theme::uses($current_theme)->load("public/themes/{$current_theme}/views/partials/home/AudioGenre", array_merge($homepage_array_data, ['data' => $SeriesGenre]) )->content() !!}
    @endif
    ...
    @if( !Auth::guest() &&  $item->video_name == 'my_play_list' && $home_settings->my_playlist == 1 ) {{-- MY PlayList --}}
        ...
        {!! Theme::uses($current_theme)->load("public/themes/{$current_theme}/views/partials/home/my-playlist", array_merge($homepage_array_data , ['data' => $MyPlaylist ]))->content() !!}
    @endif
    ...
    @if(  $item->video_name == 'Today-Top-videos' && $home_settings->Today_Top_videos == 1 )      {{-- Today Top video --}} 
        ...
        {!! Theme::uses($current_theme)->load("public/themes/{$current_theme}/views/partials/home/Today-Top-videos", array_merge($homepage_array_data , ['data' => $video_details ]))->content() !!}
    @endif

    @if(  $item->video_name == 'watchlater_videos' && $home_settings->watchlater_videos == 1 )        {{-- watchlater_videos  --}}
        {!! Theme::uses($current_theme)->load("public/themes/{$current_theme}/views/partials/home/watchlater_videos", array_merge($homepage_array_data, ['data' => $SeriesGenre]) )->content() !!}
    @endif

    @if(  $item->video_name == 'wishlist_videos' && $home_settings->wishlist_videos == 1 )        {{-- wishlist_videos  --}}
        {!! Theme::uses($current_theme)->load("public/themes/{$current_theme}/views/partials/home/wishlist_videos", array_merge($homepage_array_data, ['data' => $SeriesGenre]) )->content() !!}
    @endif

    @if( $Series_Networks_Status == 1 &&  $item->video_name == 'Series_Networks' && $home_settings->Series_Networks == 1 )      {{-- Series Networks --}} 
        {!! Theme::uses($current_theme)->load("public/themes/{$current_theme}/views/partials/home/Series-Networks", $homepage_array_data )->content() !!}
    @endif

    @if(   $item->video_name == 'Leaving_soon_videos' && $home_settings->Leaving_soon_videos == 1 )     
        {!! Theme::uses($current_theme)->load("public/themes/{$current_theme}/views/partials/home/Going-to-expiry-videos", $homepage_array_data )->content() !!}
    @endif

    @if(  $item->video_name == 'series_episode_overview' && $home_settings->series_episode_overview == 1 )    {{--  series_episode_overview  --}}
        {!! Theme::uses($current_theme)->load("public/themes/{$current_theme}/views/partials/home/series_episode_overview", array_merge($homepage_array_data , ['data' => $latest_video ]))->content() !!}
    @endif
    ...
    @if(   $item->video_name == 'EPG' && $home_settings->epg == 1 )     
        {!! Theme::uses($current_theme)->load("public/themes/{$current_theme}/views/partials/home/channel-epg", $homepage_array_data)->content() !!}
    @endif

    @if(  $item->video_name == 'Recommendation')  {{-- Recommendation --}}
        {!! Theme::uses($current_theme)->load("public/themes/{$current_theme}/views/partials/home/Top_videos", array_merge($homepage_array_data , ['data' => $top_most_watched ]))->content() !!}
        {!! Theme::uses($current_theme)->load("public/themes/{$current_theme}/views/partials/home/most_watched_country", array_merge($homepage_array_data , ['data' => $Most_watched_country ]))->content() !!}
        {!! Theme::uses($current_theme)->load("public/themes/{$current_theme}/views/partials/home/most_watched_user", array_merge($homepage_array_data , ['data' => $most_watch_user ]))->content() !!}
    @endif
@empty

...
{{-- video-js Style --}}
    ...
    <link href="{{ asset('public/themes/theme1/assets/css/video-js/videojs.min.css') }}" rel="stylesheet" >
                        ...
                        @if(isset($subtitles) && count($subtitles) > 0)
                            @foreach($subtitles as $subtitles_file)
                                <track kind="subtitles" src="{{ $subtitles_file->url }}"
                                    ...
                                    label="{{ $subtitles_file->shortcode }}" @if($loop->first) theme1 @endif>
                            @endforeach
                        @endif
                    @endif
                </video>
...
@if(isset($setting) && $setting->video_clip_enable == 1 && count($videoURl) > 0)
    @php include public_path('themes/theme1/views/video-js-Player/video/Concat_Player_Script.blade.php'); @endphp
@else
    @php include public_path('themes/theme1/views/video-js-Player/video/player_script.blade.php'); @endphp
@endif

...
<!-- For Guest users -->
<?php if (
  Auth::guest() && $video->access == "guest" && empty ($video->ppv_price)
  ...
  <?php } elseif (Auth::guest() && $video->access == "guest" && empty ($video->ppv_price) && !empty ($video->path) || Auth::guest() && $video->access == "guest" && $video->path != "public" && empty ($video->ppv_price)) { ?>
    ...
    <input type="hidden" id="hls_m3u8" name="hls_m3u8"
      value="<?php echo URL::to('/storage/app/public/') . '/' . $video->path . '.m3u8'; ?>">
  <?php } else {
  if (!empty ($video->path) && $video->path != "public") {
    $hls = "hls";
  } else {
    $hls = "";
  }
  ?>
    ...
    <div id="subscribers_only"
      style="background: url(<?= URL::to('/') . '/public/uploads/images/' . $video->player_image ?>); background-repeat: no-repeat; background-size: cover; height: 400px; margin-top: 20px;">
      ...
      <h2>
        ...
        if ($video->access == 'subscriber') { ?>
          <p style="text-align:center">
            <?= __('Sorry, this video is only available to Subscribers') ?>
          </p>
          <?php
        } elseif ($video->access == 'registered') { ?>
          <p style="text-align:center">
            <?= __('Sorry, this video is only available to Registered') ?>
          </p>
          <?php
        } elseif ($video->access == 'ppv') { ?>
          <p style="text-align:center">
            <?= __('Sorry, this video is only available to PPV users') ?>
          </p>
          <?php
        }
        ?>
        ...
        <?php if (Auth::guest() && ($video->access == "registered")) { ?>
          ...
          <form method="get" action="<?= URL::to('signup') ?>">
            <button class="btn btn-primary" id="button">
              <?= __('Become a ' . $video->access . ' to watch this video') ?>
            </button>
          </form>
        ...
        <div class="col-sm-9 col-md-9 col-xs-12">
          ...
          <?php } else { ?>
            <h1 class="trending-text big-title text-uppercase mt-3">
              <?php echo (strlen($video->title) > 15) ? substr($video->title, 0, 80) . '...' : $video->title;
              if (Auth::guest()) {
              } ?>
            </h1>
          <?php } ?>
        ...
        <div class="col-sm-6 col-md-6 col-xs-12">
          <ul class="list-inline p-0 mt-4 share-icons music-play-lists">
            ...
            <li><span class="watchlater <?php if (isset ($watchlatered->id)): ?>active<?php endif; ?>"
                data-authenticated="<?= !Auth::guest() ?>" data-videoid="<?= $video->id ?>"><i <?php if (isset ($watchlatered->id)): ?> class="ri-add-circle-fill" <?php else: ?> class="ri-add-circle-line"
                  <?php endif; ?>></i></span></li>
            <!-- Wishlist -->
          ...
          <ul class="list-inline p-0 mt-4 rental-lists">
            ...
            <li>
              ...
              if (($user->role != "subscriber" && $user->role != "admin")) { ?>
                <a href="<?php echo URL::to('/becomesubscriber'); ?>"><span class="view-count btn btn-primary subsc-video">
    ...
    <div class=" page-height">
      <!-- <div id="watch_trailer" class="fitvid" atyle="z-index: 9999;">
        ...
        <?php if ($video->trailer_type != null && $video->trailer_type == "video_mp4" || $video->trailer_type == "mp4_url") { ?>
          ...
          <video class="videoPlayer1" controls data-setup='{"controls": true, "aspectRatio":"16:9", "fluid": true}'
            type="video/mp4" src="<?php echo $video->trailer; ?>">
          </video>
        <?php } elseif ($video->trailer_type != null && $video->trailer_type == "m3u8") { ?>

          <video <?= $autoplay ?> id="videos" class=""
            poster="<?= URL::to('/') . '/public/uploads/images/' . $video->player_image ?>" controls
      ...
      <div class="col-sm-12 mt-4 p-0" style="margin-top: 2%;">
        <?php if (!empty ($video->description) && $settings->show_description == 1) { ?>
          ...
          <div class="text-white">
            <p class="trending-dec w-100 mb-0 text-white mt-3">

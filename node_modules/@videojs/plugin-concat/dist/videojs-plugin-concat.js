...
(function (global, factory) {
  ...
  (global = global || self, global.pluginConcat = factory(global.videojs, global.window));
}(this, function (videojs, window$2) { 'use strict';
  ...
  var requestAll = function requestAll(urls, callback) {
    ...
    urls.forEach(function (url) {
      var request = videojs.xhr(url, function (err, response) {
        if (didError) {
          return;
        }

        pendingRequests.splice(pendingRequests.indexOf(request), 1);
        var responseStatusIsSuccess = response && (response.statusCode === 200 || response.statusCode === 206 || response.statusCode === 0);
        ...
        if (err || !responseStatusIsSuccess) {
          ...
          didError = true; // abort any pending requests, since one error is enough to fail the entire set
  ...
  var resolveUrl = function resolveUrl(baseUrl, relativeUrl) {
    ...
    if (!/\/\//i.test(baseUrl)) {
      baseUrl = urlToolkit.buildAbsoluteURL(window$2.location.href, baseUrl);
    }
  ...
  var mediaTypes = createCommonjsModule$1(function (module, exports) {
  ...
  var mediaGroups = createCommonjsModule$1(function (module, exports) {
  ...
  var addPropertiesToMedia = function addPropertiesToMedia(media) {
    ...
    media.attributes = media.attributes || {}; // mpd-parser provides resolved URIs for all segments, but m3u8-parser does not. To
  ...
  var parseManifest = function parseManifest(_ref) {
    ...
    var type = mediaTypes_1(mimeType);
    var manifest = type === 'dash' ? parse(manifestString, {
      manifestUri: url,
      clientOffset: 0
    }) : parseM3u8(manifestString); // Although VHS adds resolvedUri properties to the playlists and segments,
  ...
  var chooseVideoPlaylists = function chooseVideoPlaylists(manifestsPlaylists, targetVerticalResolution) {
    return manifestsPlaylists.map(function (manifestPlaylists) {
      ...
      return manifestPlaylists.reduce(function (acc, playlist) {
        ...
        return Math.abs(playlist.attributes.BANDWIDTH - DEFAULT_BANDWIDTH) < Math.abs(acc.attributes.BANDWIDTH - DEFAULT_BANDWIDTH) ? playlist : acc;
      }, null);
    });
  };
  /**
  ...
  var chooseAudioPlaylists = function chooseAudioPlaylists(manifestObjects, videoPlaylists) {
    ...
    var numExpectedPlaylists = manifestObjects.length;
    var audioPlaylists = [];

    for (var i = 0; i < numExpectedPlaylists; i++) {
      var manifestObject = manifestObjects[i];
      var videoPlaylist = videoPlaylists[i];

      if (!videoPlaylist.attributes.AUDIO || !manifestObject.mediaGroups.AUDIO[videoPlaylist.attributes.AUDIO]) {
        // unable to find a matching audio object
        continue;
      }

      var manifestAudioPlaylists = manifestObject.mediaGroups.AUDIO[videoPlaylist.attributes.AUDIO];
      var audioPlaylistNames = Object.keys(manifestAudioPlaylists);

      for (var j = 0; j < audioPlaylistNames.length; j++) {
        var audioPlaylist = manifestAudioPlaylists[audioPlaylistNames[j]];

        if (audioPlaylist.default && ( // some audio playlists are merely identifiers for muxed audio, don't include
        ...
        audioPlaylist.resolvedUri || audioPlaylist.playlists)) {
          audioPlaylists.push(audioPlaylist.playlists ? audioPlaylist.playlists[0] : audioPlaylist);
          break;
        }
      }
  ...
  var codecs = createCommonjsModule$1(function (module, exports) {
  ...
  var codecsForPlaylists = function codecsForPlaylists(manifest) {
    ...
    var playlists = manifest.playlists ? manifest.playlists : [manifest];
    return playlists.reduce(function (acc, playlist) {
      ...
      var codecs = codecs_3(playlist.attributes.CODECS);

      if (codecs.codecCount !== 2 && playlist.attributes.AUDIO) {
        var audioProfile = codecs_1(manifest, playlist.attributes.AUDIO);
  ...
  var getAudioAndVideoTypes = function getAudioAndVideoTypes(_ref) {
    ...
    var audioAndVideoTypes = [];

    for (var i = 0; i < videoPlaylists.length; i++) {
      var videoPlaylist = videoPlaylists[i];
      var manifestObject = manifestObjects[i];
      var playlistToCodecsMap = codecsForPlaylists(manifestObject);
      var codecs = playlistToCodecsMap[videoPlaylist.resolvedUri]; // HLS media playlist, or an HLS master without the CODECS attribute attached to
      // playlists
  ...
  var removeUnsupportedPlaylists = function removeUnsupportedPlaylists(manifestObjects) {
    ...
    return manifestObjects.map(function (manifestObject) {
      ...
      var playlistToCodecsMap = codecsForPlaylists(manifestObject); // handle master and media playlists

      var playlists = manifestObject.playlists ? manifestObject.playlists : [manifestObject];
      return playlists.filter(function (playlist) {
        var codecs = playlistToCodecsMap[playlist.resolvedUri]; // Allow playlists with no specified codecs to pass through. Although the playlists
        ...
        if (window$2.MediaSource && window$2.MediaSource.isTypeSupported && // ignore demuxed audio for the MSE support check to mirror VHS' check
        !window$2.MediaSource.isTypeSupported('video/mp4; codecs="' + codecs_2(playlist.attributes.CODECS) + '"')) {
          return false;
        }
  ...
  var combinePlaylists = function combinePlaylists(_ref) {
    var playlists = _ref.playlists,
        ...
        uriSuffix = _ref$uriSuffix === void 0 ? '' : _ref$uriSuffix;
    var combinedSegments = playlists.reduce(function (acc, playlist) {
      var firstNewSegmentIndex = acc.length; // need to clone because we're modifying the segment objects

      var clonedSegments = JSON.parse(JSON.stringify(playlist.segments));
      var concatenatedSegments = acc.concat(clonedSegments); // don't add a discontinuity to the first segment
    ...
    var maxBandwidth = playlists.reduce(function (acc, playlist) {
      if (playlist.attributes && playlist.attributes.BANDWIDTH && (!acc || playlist.attributes.BANDWIDTH > acc)) {
        return playlist.attributes.BANDWIDTH;
      }

      return acc;
    }, null); // Because the codecs may be different (but compatible), use the first defined set, if
    // available.

    var codecs = playlists.reduce(function (acc, playlist) {
      if (acc) {
        return acc;
      }

      return playlist.attributes ? playlist.attributes.CODECS : null;
    }, null); // Attributes used are a subset of
    // https://tools.ietf.org/html/draft-pantos-http-live-streaming-23#section-4.3.4.2
    ...
    for (var i = 0; i < combinedPlaylist.segments.length; i++) {
      var segment = combinedPlaylist.segments[i];
  ...
  var constructMasterManifest = function constructMasterManifest(_ref2) {
    ...
    var audioPlaylists = audioPlaylist ? [audioPlaylist] : null; // VHS playlist arrays have properties with the playlist URI in addition to the standard
  ...
  var WIDEVINE_KEY_SYSTEM_STRING = 'com.widevine.alpha';
  /**
  ...
  var createInitializeKeySystemsFunction = function createInitializeKeySystemsFunction(_ref2) {
    ...
    var containsEncryptedSource = keySystems.reduce(function (acc, keySystem) {
    ...
    var initializeKeySystemsFunctions = []; // video and audio playlist counts should match, so either length can be used

    for (var i = 0; i < videoPlaylists.length; i++) {
      var keySystemsConfig = keySystems[i]; // this handles both the case of no DRMed content and DRMed mixed with unencrypted
      // content
      ...
      var widevineKeySystem = keySystemsConfig[WIDEVINE_KEY_SYSTEM_STRING]; // at the moment, only widevine is supported

      if (!widevineKeySystem) {
        continue;
      }

      if (!audioAndVideoTypes[i] || !(audioAndVideoTypes[i].video || audioAndVideoTypes[i].audio)) {
      ...
      var url = typeof widevineKeySystem === 'object' ? widevineKeySystem.url : widevineKeySystem;
      var getLicense = typeof widevineKeySystem === 'object' ? widevineKeySystem.getLicense : null;
      var videoPlaylist = videoPlaylists[i];
      var audioPlaylist = audioPlaylists[i];
      var videoPssh = videoPlaylist.contentProtection && videoPlaylist.contentProtection[WIDEVINE_KEY_SYSTEM_STRING] ? videoPlaylist.contentProtection[WIDEVINE_KEY_SYSTEM_STRING].pssh : null;
      var audioPssh = audioPlaylist.contentProtection && audioPlaylist.contentProtection[WIDEVINE_KEY_SYSTEM_STRING] ? audioPlaylist.contentProtection[WIDEVINE_KEY_SYSTEM_STRING].pssh : null;
      var audioType = audioAndVideoTypes[i].audio;
      var videoType = audioAndVideoTypes[i].video;
      var commonOptions = {
      ...
      if (videoPssh) {
        var options = videojs.mergeOptions(commonOptions, {
          pssh: videoPssh
        });
      ...
      if (audioPssh) {
        var _options = videojs.mergeOptions(commonOptions, {
          pssh: audioPssh
        });
  ...
  var resolvePlaylists = function resolvePlaylists(_ref) {
    ...
    var playlistUris = playlists // if the segments are already resolved, don't need to request (DASH case)
    .filter(function (playlist) {
      return !playlist.segments;
    }).map(function (playlist) {
      return playlist.resolvedUri;
    });
    var preResolvedPlaylists = playlists.filter(function (playlist) {
      return playlist.segments;
    });
    var origPlaylistsToParsed = {};
    ...
    var uriToPlaylistsMap = {};
    var uriToMimeTypeMap = {};

    for (var i = 0; i < playlists.length; i++) {
      var playlist = playlists[i]; // it's possible for the caller to concat two of the same video together
    ...
    requestAll(playlistUris, function (err, responses) {
      ...
      for (var _i = 0; _i < playlistUris.length; _i++) {
        var uri = playlistUris[_i];
        var origPlaylists = uriToPlaylistsMap[uri];
        var playlistString = responses[uri];
        var mimeType = uriToMimeTypeMap[uri];

        var _playlist = parseManifest({
          url: uri,
        ...
        for (var j = 0; j < origPlaylists.length; j++) {
          var origPlaylist = origPlaylists[j];
  ...
  var concatenateManifests = function concatenateManifests(_ref2) {
    ...
    var manifestObjects = manifests.map(parseManifest);
    var supportedPlaylists = removeUnsupportedPlaylists(manifestObjects);
    ...
    var videoPlaylists = chooseVideoPlaylists(supportedPlaylists, targetVerticalResolution); // A rendition with demuxed audio can't be concatenated with a rendition with muxed
    ...
    var audioPlaylists = chooseAudioPlaylists(manifestObjects, videoPlaylists);
    var allPlaylists = videoPlaylists.concat(audioPlaylists); // To correctly set the mime types for all playlists, we have to use the mime types
    ...
    var mimeTypes = manifests.map(function (manifest) {
    ...
    resolvePlaylists({
      ...
      callback: function callback(err, resolvedPlaylistsMap) {
        ...
        allPlaylists.forEach(function (playlist) {
          var resolvedPlaylist = resolvedPlaylistsMap[playlist.resolvedUri];
          playlist.segments = resolvedPlaylist.segments; // used for CDM specific settings for DRM/EME

          playlist.contentProtection = resolvedPlaylist.contentProtection;
        });
        var combinedVideoPlaylist = combinePlaylists({
          playlists: videoPlaylists
        });
        var combinedAudioPlaylist = audioPlaylists.length ? combinePlaylists({
          playlists: audioPlaylists,
          uriSuffix: '-audio'
        }) : null;
        var manifestObject = constructMasterManifest({
        ...
        var audioAndVideoTypes = getAudioAndVideoTypes({
  ...
  var concatenateVideos = function concatenateVideos(_ref3) {
    ...
    var errorMessage = getProvidedManifestsError(manifests);
    ...
    var urls = manifests.map(function (manifestObject) {
    ...
    requestAll(urls, function (err, responses) {
      ...
      var orderedManifests = manifests.map(function (manifestObject) {
        return {
  ...
  registerPlugin('concat', concat); // Include the version number.

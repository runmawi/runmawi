...
class AdminUsersController extends Controller
{
    ...
    public function store(Request $request)
    {
        ...
        if (
            $input['role'] == 'subscriber' && !empty($input['plan'])
    ...
    public function update(Request $request)
    {
        ...
        DB::table('users')->where('id', $id)->update(
            [
                ...
                'otp' => !empty($input['otp']) ? $input['otp'] : "1234",
                'terms' => $input['terms'],
                'avatar' => $avatar_image,
        ...
        if ($current_date > $duedate) {
            ...
            $user_Revenue = User::join('ppv_purchases', 'users.id', '=', 'ppv_purchases.user_id')
                ...
                ->get([
                    ...
                    'subscription_plans.plans_name',
                    'users.role',
                    'ppv_purchases.total_amount',
                    'ppv_purchases.created_at',
                    'ppv_purchases.audio_id',
    ...
    public function SubscriberRevenueStartDateRecord(Request $request)
    {
        ...
        if (!empty($subscriber_Revenue)) {
            foreach ($subscriber_Revenue as $key => $row) {
                ...
                if (!empty($row->plans_name)) {
                    $plans_name = $row->plans_name;
                } else {
                    $plans_name = "Registered";
                }
                if (!empty($row->audio_id)) {
                    $Content = 'Audio';
                } elseif (!empty($row->video_id)) {
                    $Content = 'Video';
                } elseif (!empty($row->live_id)) {
                    $Content = 'Live';
                } else {
                    $Content = 'All Content';
                }
                if (@$row->phoneccode->phonecode == $row->ccode) {
                    $country_name = @$row->phoneccode->country_name;
                } else {
                    $country_name = "No Country Added";
                }
                if (!empty($row->card_type)) {
                    $card_type = @$row->card_type;
                } else {
                    $card_type = "No Transaction";
                }
    ...
    public function UserSubscriberExportCsv(Request $request)
    {
        ...
        if (count($subscriber_Revenue) > 0) {
            foreach ($subscriber_Revenue as $each_user) {
                if (!empty($each_user->stripe_id)) {
                    $stripe_id = $each_user->stripe_id;
                } else {
                    $stripe_id = 'No REF';
                }
                if (!empty($each_user->plans_name)) {
                    $plans_name = $each_user->plans_name;
                } else {
                    $plans_name = "Registered";
                }
                if (!empty($each_user->audio_id)) {
                    $Content = 'Audio';
                } elseif (!empty($each_user->video_id)) {
                    $Content = 'Video';
                } elseif (!empty($each_user->live_id)) {
                    $Content = 'Live';
                } else {
                    $Content = 'All Content';
                }
                if (@$each_user->phoneccode->phonecode == $each_user->ccode) {
                    $country_name = @$each_user->phoneccode->country_name;
                } else {
                    $country_name = "No Country Added";
                }
                if (!empty($each_user->card_type)) {
                    $card_type = @$each_user->card_type;
                } else {
                    $card_type = "No Transaction";
                }
        ...
        if (
            ...
            if (!empty($start_time) && empty($end_time)) {
                $player_videos = PlayerAnalytic::join('users', 'users.id', '=', 'player_analytics.user_id')
                    ...
                    ->groupBy('player_analytics.videoid')
                    ->get([
                        'player_analytics.videoid',
                        'player_analytics.user_id',
                        'users.username',
                        'videos.title',
                        'videos.slug',
    ...
    public function updateStripeStatus(Request $request)
    {
        ...
        try {
            ...
            \Mail::send(
                'emails.subscriptionStatusUpdate',
                array(

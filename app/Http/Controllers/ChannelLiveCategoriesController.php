...
class ChannelLiveCategoriesController extends Controller
{
    public function Channelindex()
    {
        ...
        if (!empty($package) && $package == "Pro" || !empty($package) && $package == "Business")
        {
            ...
            $user_id = $user->id;
            $categories = LiveCategory::where('user_id', '=', $user_id)->where('uploaded_by', 'Channel')
                ->where('parent_id', '=', 0)
                ->get();

            $allCategories = LiveCategory::where('user_id', '=', $user_id)->where('uploaded_by', 'Channel')
                ->get();
    ...
    public function Channelstore(Request $request)
    {
        ...
        if (!empty($package) && $package == "Pro" || !empty($package) && $package == "Business")
        {
            ...
            $LiveCategory->save();
            return back()
                ->with('message', 'New Category added successfully.');
        }
        else
        {
            return Redirect::to('/blocked');
    ...
    public function Channeledit($id)
    {
        ...
        if (!empty($package) && $package == "Pro" || !empty($package) && $package == "Business")
        {
            $user = Session::get('channel');
            $user_id = $user->id;
            $categories = LiveCategory::where('id', '=', $id)->where('uploaded_by', 'Channel')
                ->get();
    ...
    public function Channelupdate(Request $request)
    {
        ...
        if (!empty($package) && $package == "Pro" || !empty($package) && $package == "Business")
        {
            ...
            return Redirect::to('channel/livestream/categories')
                ->with(array(

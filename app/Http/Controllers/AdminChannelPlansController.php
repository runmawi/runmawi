...
class AdminChannelPlansController extends Controller
{
    ...
    public function subscriptionindex()
    {
        ...
        if ($current_date > $duedate) {
            ...
            $plans_data = ChannelSubscriptionPlan::all();
            ...
            return view('admin.channel_subscription_plans.index', $data);
        }
    }
    ...
    public function subscriptionedit($id)
    {
        ...
        if ($current_date > $duedate) {
            ...
            // $edit_plan = ChannelSubscriptionPlan::find($id);
            $edit_plan = ChannelSubscriptionPlan::where('id', $id)->get();
            if (!empty($edit_plan[0]->plans_name)) {
                $edit_plan = ChannelSubscriptionPlan::where('plans_name', $edit_plan[0]->plans_name)->get();
            }
            ...
            return view('admin.channel_subscription_plans.edit', $data);
        }
    }

    public function subscriptiondelete($id)
    {
        ChannelSubscriptionPlan::where('id', $id)->delete();
        ...
        if (!empty($request->plan_id)) {
            ...
            foreach ($request->plan_id as $key => $value) {
                ...
                foreach ($request->type as $typekey => $types) {
                    ...
                    if ($typekey == $key) {
                        $new_plan = new ChannelSubscriptionPlan();
                        $new_plan->type = $types;
    ...
    public function subscriptionupdate(Request $request)
    {
        ...
        foreach ($subscription_plan_name as $value) {
            $plans = ChannelSubscriptionPlan::where('subscription_plan_name', $value)->first();
        ...
        return Redirect::to('admin/channel-subscription-plans/')->with(['note' => 'You have been successfully Added New Plan', 'note_type' => 'success']);
    }

...
class ChannelHomeController extends Controller
{
    ...
    public function ChannelHome($slug)
    {
        ...
        if(!empty($channel)){
            ...
            $livetreams = LiveStream::where('active', 1)->where('status',1)->where('user_id', $channel->id)
                                            ->where('uploaded_by', 'Channel')->orderBy('created_at', 'DESC')
                                            ->get();

            $latest_videos = $FrontEndQueryController->latest_videos()->filter(function ($latest_videos) use ($channel) {
                if ( $latest_videos->user_id == $channel->id && $latest_videos->uploaded_by == "Channel" ) {
                    return $latest_videos;
                }
            });

            $latest_series = $FrontEndQueryController->latest_Series()->filter(function ($latest_Series) use ($channel) {
                if ( $latest_Series->user_id == $channel->id && $latest_Series->uploaded_by == "Channel" ) {
                    return $latest_Series;
                }
            });

            $latest_audios = $FrontEndQueryController->latest_audios()->filter(function ($latest_audios) use ($channel) {
            ...
            $featured_videos = $FrontEndQueryController->featured_videos()->filter(function ($featured_videos) use ($channel) {
                if ( $featured_videos->user_id == $channel->id && $featured_videos->uploaded_by == "Channel" ) {
                    return $featured_videos;
                }
            });

            $trending_videos = $FrontEndQueryController->trending_videos()->filter(function ($trending_videos) use ($channel) {
                if ( $trending_videos->user_id == $channel->id && $trending_videos->uploaded_by == "Channel" ) {
                    return $trending_videos;
                }
            });

            $featured_episodes = $FrontEndQueryController->featured_episodes()->filter(function ($featured_episodes) use ($channel) {
                if ( $featured_episodes->user_id == $channel->id && $featured_episodes->uploaded_by == "Channel" ) {
                    return $featured_episodes;
                }
            });
            
            $genre_video_display = $FrontEndQueryController->genre_video_display()->filter(function ($genre_video_display) use ($channel) {
                if ( $genre_video_display->user_id == $channel->id && $genre_video_display->uploaded_by == "Channel" ) {
                    return $genre_video_display;
                }
            });

            $albums = $FrontEndQueryController->AudioAlbums()->filter(function ($albums) use ($channel) {
                if ( $albums->user_id == $channel->id && $albums->uploaded_by == "Channel" ) {
                    return $albums;
                }
            });

            $latest_episodes = $FrontEndQueryController->latest_episodes()->filter(function ($latest_episodes) use ($channel) {
                if ( $latest_episodes->user_id == $channel->id && $latest_episodes->uploaded_by == "Channel" ) {
                    return $latest_episodes;
                }
            });
            
            $artist = $FrontEndQueryController->artist()->filter(function ($artist) use ($channel) {
                if ( $artist->user_id == $channel->id && $artist->uploaded_by == "Channel" ) {
                    return $artist;
                }
            });

            $LiveCategory = $FrontEndQueryController->LiveCategory()->filter(function ($LiveCategory) use ($channel) {
                if ( $LiveCategory->user_id == $channel->id && $LiveCategory->uploaded_by == "Channel" ) {
                    return $LiveCategory;
                }
            });

            $LiveEventArtist = $FrontEndQueryController->LiveEventArtist()->filter(function ($LiveEventArtist) use ($channel) {
                if ( $LiveEventArtist->user_id == $channel->id && $LiveEventArtist->uploaded_by == "Channel" ) {
                    return $LiveEventArtist;
                }
            });

            $trending_audios = $FrontEndQueryController->trending_audios()->filter(function ($trending_audios) use ($channel) {
                if ( $trending_audios->user_id == $channel->id && $trending_audios->uploaded_by == "Channel" ) {
                    return $trending_audios;
                }
            });

            $live_banners = $FrontEndQueryController->live_banners()->filter(function ($live_banners) use ($channel) {
                if ( $live_banners->user_id == $channel->id && $live_banners->uploaded_by == "Channel" ) {
                    return $live_banners;
                }
            });

            $video_banners = $FrontEndQueryController->video_banners()->filter(function ($video_banners) use ($channel) {
                if ( $video_banners->user_id == $channel->id && $video_banners->uploaded_by == "Channel" ) {
                    return $video_banners;
                }
            });

            $series_sliders = $FrontEndQueryController->series_sliders()->filter(function ($series_sliders) use ($channel) {
                if ( $series_sliders->user_id == $channel->id && $series_sliders->uploaded_by == "Channel" ) {
                    return $series_sliders;
                }
            });

            $live_event_banners = $FrontEndQueryController->live_event_banners()->filter(function ($live_event_banners) use ($channel) {
                if ( $live_event_banners->user_id == $channel->id && $live_event_banners->uploaded_by == "Channel" ) {
                    return $live_event_banners;
                }
            });

            $Episode_sliders = $FrontEndQueryController->Episode_sliders()->filter(function ($Episode_sliders) use ($channel) {
            ...
            $VideoCategory_banner = $FrontEndQueryController->VideoCategory_banner()->filter(function ($VideoCategory_banner) use ($channel) {
                if ( $VideoCategory_banner->user_id == $channel->id && $VideoCategory_banner->uploaded_by == "Channel" ) {
                    return $VideoCategory_banner;
                }
            });

            // $Most_watched_videos_users = $FrontEndQueryController->Most_watched_videos_users()->filter(function ($Most_watched_videos_users) use ($channel) {
            ...
            // $Most_watched_videos_site = $FrontEndQueryController->Most_watched_videos_site()->filter(function ($Most_watched_videos_site) use ($channel) {
            ...
            $AudioCategory = $FrontEndQueryController->AudioCategory()->filter(function ($AudioCategory) use ($channel) {
                if ( $AudioCategory->user_id == $channel->id && $AudioCategory->uploaded_by == "Channel" ) {
                    return $AudioCategory;
                }
            });
            
            // $preference_genres = $FrontEndQueryController->preference_genres()->filter(function ($preference_genres) use ($channel) {
            //     if ( $preference_genres->user_id == $channel->id && $preference_genres->uploaded_by == "Channel" ) {
            //         return $preference_genres;
            //     }
            // });

            // $preference_language = $FrontEndQueryController->preference_language()->filter(function ($preference_language) use ($channel) {

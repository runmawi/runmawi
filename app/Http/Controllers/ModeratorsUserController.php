...
class ModeratorsUserController extends Controller
{
    ...
    public function store(Request $request)
    {
        ...
        if (!empty($data['password_hash']))
        {
            ...
            {
                $input = $request->all();
                $role = ModeratorsRole::where('id', '=', $request->user_role)
                    ->get();
                // echo "<pre>";
                // print_r($role);
                // exit();
                ...
                $request->validate(['email_id' => 'required|email|unique:moderators_users,email', 'password' => 'min:6', 'confirm_password' => 'required_with:password|same:password|min:6']);
    ...
    public function CPPModeratorsApproval($id)
    {
        ...
        $users->status = 1;
        $users->save();
        Mail::send('emails.cpp_approved', array() , function ($message) use ($users)
        {
            $message->from(AdminMail() , 'Flicknexs');
            $message->to($users->email)
                ->subject('Approved As Moderator');
        });
    ...
    public function update(Request $request)
    {
        ...
        if (!empty($data['password_hash']))
        {
            ...
            if ($package == "Pro" || $package == "Business" || $package == "" && Auth::User()->role == "admin")
            {
                ...
                return back()
                    ->with('message', 'Successfully User Updated!.');
    ...
    public function RoleUpdate(Request $request)
    {
        ...
        if (!empty($data['password_hash']))
        {
            ...
            if ($package == "Pro" || $package == "Business" || $package == "" && Auth::User()->role == "admin")
            {
                ...
                $moderatorrole->save();
                return back()
                    ->with('message', 'Successfully Roles Updated!.');
    ...
    public function plans(Request $request)
    {
        ...
        $c_count = Plan::where('plan_id', '=', $new_plan->plan_id)
            ->count();
        if ($c_count == 0)
        {
            $new_plan->save();
    ...
    public function paypalplans(Request $request)
    {
        ...
        $c_count = PaypalPlan::where('plan_id', '=', $new_plan->plan_id)
            ->count();
        if ($c_count == 0)
        {
            $new_plan->save();
    ...
    public function edit_series(Request $request)
    {
        ...
        $seasons = SeriesSeason::where('series_id', '=', $id)->with('episodes')
            ->get();
        $data = array(
            'series' => $series,
            'seasons' => $seasons,
    ...
    public function plans_update(Request $request)
    {
        ...
        $plans_count = Plan::where('plans_name', '=', $plans->plans_name)
            ->count();
        $plans->save();
    ...
    public function LanguageUpdate(Request $request)
    {
        ...
        return back()
            ->with('success', 'New Language Updated successfully.');
    ...
    public function Revenue()
    {
        ...
        $moderators_users = User::join('ppv_purchases', 'users.id', '=', 'ppv_purchases.user_id')->join('moderators_users', 'ppv_purchases.moderator_id', '=', 'moderators_users.id')
            ->leftjoin('videos', 'videos.id', '=', 'ppv_purchases.video_id')
            ->leftjoin('audio', 'audio.id', '=', 'ppv_purchases.audio_id')
            ->groupBy('ppv_purchases.user_id')
            ->orderBy('ppv_purchases.created_at')
    ...
    public function CPPStartDateRevenue(Request $request)
    {
        ...
        if (!empty($start_time) && empty($end_time))
        {
            ...
            $view_count = DB::table('ppv_purchases')->join('moderators_users', 'ppv_purchases.moderator_id', '=', 'moderators_users.id')
                ...
                ->leftjoin('audio', 'audio.id', '=', 'ppv_purchases.audio_id')
                ->leftjoin('live_streams', 'live_streams.id', '=', 'ppv_purchases.live_id')
                ->whereDate('ppv_purchases.created_at', '>=', $start_time)->get(['ppv_purchases.user_id', DB::raw('sum(videos.views) as videos_views') , \DB::raw("sum(audio.views) as audio_count")
            //  \DB::raw("sum(live_streams.views) as audio_count")
            ]);
            if (count($view_count) > 0)
            {
            ...
            $total_users = User::join('ppv_purchases', 'users.id', '=', 'ppv_purchases.user_id')->join('moderators_users', 'ppv_purchases.moderator_id', '=', 'moderators_users.id')
                ->leftjoin('videos', 'videos.id', '=', 'ppv_purchases.video_id')
                ->leftjoin('audio', 'audio.id', '=', 'ppv_purchases.audio_id')
                ->whereDate('ppv_purchases.created_at', '>=', $start_time)->groupBy('ppv_purchases.user_id')
                ->orderBy('ppv_purchases.created_at')
    ...
    public function CPPEndDateRevenue(Request $request)
    {
        ...
        if (!empty($start_time) && !empty($end_time))
        {
            ...
            $total_Revenue = User::join('ppv_purchases', 'users.id', '=', 'ppv_purchases.user_id')->join('moderators_users', 'ppv_purchases.moderator_id', '=', 'moderators_users.id')
                ->whereBetween('ppv_purchases.created_at', [$start_time, $end_time])->groupBy('ppv_purchases.user_id')
                ->orderBy('ppv_purchases.created_at')
                ->get(['ppv_purchases.user_id', DB::raw('sum(ppv_purchases.moderator_commssion) as count') , \DB::raw("MONTHNAME(ppv_purchases.created_at) as month_name") ]);
            ...
            $view_count = DB::table('ppv_purchases')->join('moderators_users', 'ppv_purchases.moderator_id', '=', 'moderators_users.id')
                ...
                ->leftjoin('audio', 'audio.id', '=', 'ppv_purchases.audio_id')
                ->leftjoin('live_streams', 'live_streams.id', '=', 'ppv_purchases.live_id')
                ->whereBetween('ppv_purchases.created_at', [$start_time, $end_time])->get(['ppv_purchases.user_id', DB::raw('sum(videos.views) as videos_views') , \DB::raw("sum(audio.views) as audio_count")
            //  \DB::raw("sum(live_streams.views) as audio_count")
            ]);
            if (count($view_count) > 0)
            {
    ...
    public function CPPExportCsv(Request $request)
    {
        ...
        fputcsv($handle, ["Content Partner", "Total Views", "% Shared Commission", ]);
        if (count($total_users) > 0)
        {
            foreach ($total_users as $each_user)
            {
                ...
                fputcsv($handle, [$each_user->username, $view_count, $each_user->count,

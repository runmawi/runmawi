...
if(!empty($package) && $package== "Pro" || !empty($package) && $package == "Business" ){
      $user = Session::get('channel'); 
        ...
        if(!empty($search_value)):
            $videos = Video::where('title', 'LIKE', '%'.$search_value.'%')->where('user_id','=',$id)->orderBy('created_at', 'desc')->paginate(9);
        else:
            $videos = Video::where('user_id','=',$id)->where('uploaded_by','Channel')->orderBy('created_at', 'DESC')->paginate(9);
        endif;
...
//            }  
        ...
        if(!empty($data['language'])){
            ...
            if(!empty($language_id)){
                $languagevideo = LanguageVideo::where('video_id', $video->id)->delete();

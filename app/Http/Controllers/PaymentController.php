...
class PaymentController extends Controller
{
  ...
  public function BecomeSubscriber()
  {
    ...
    if (!Auth::guest()) {
      ...
      if ($signup_checkout == 1) {
        ...
        $is_subscribed = Subscriber::where('user_id', Auth::id())
            ->whereDate('end_date', '>=', Carbon::now())->where('payment_status','active')
            ->first();
        ...
        if(!$is_subscribed){
          $pendingSubscription = Subscriber::where('user_id', Auth::id())
  ...
  public function RazorPaySubscription($id)
  {
      ...
      $plan_details = DB::table('subscription_plans')->where('id', $id)->first();

      if (!$plan_details || empty($plan_details->plan_id)) {
          abort(404, 'Plan not found or Razorpay plan_id missing.');
      }
      ...
      return Theme::view('Razorpay.subscription_checkout', [
          'plan_details'    => $plan_details,
          'subscription_id' => $subscription->id,
          'rzp_key'         => $razorpaykeyId,
      ]);
  }

    ...
    try {
        ...
        $AdsVideosPre = AdsEvent::Join('advertisements','advertisements.id','=','ads_events.ads_id')
            ...
            ->get()->map->only('ads_path','ads_video')->map(function ($item) {
                $item['ads-videos_url'] = URL::to('public/uploads/AdsVideos/'.$item['ads_video']);
                return $item;
            });
        ...
        $response = array(
            'status'=>'true',
            'Ads-Videos-Pre' => $Ads_Videos_Pre,
            'Ads-Videos-Mid' => $Ads_Videos_Mid,
            'Ads-Videos-post' => $Ads_Videos_Post,
        );
    }
    catch (\Throwable $th) {
        $response = array(
      ...
      try{
        ...
        if($HomeSetting->Recommendation == 1){
          ...
          $most_watch_user = RecentView::select('video_id', 'videos.*', DB::raw('COUNT(video_id) AS count'))
                ->join('videos', 'videos.id', '=', 'recent_views.video_id')->groupBy('video_id');

          $most_watch_user = $multiuser != null ?  $most_watch_user->where('recent_views.sub_user', $multiuser) :  $most_watch_user->where('recent_views.user_id', Auth::user()->id) ;

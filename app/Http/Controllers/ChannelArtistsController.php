            ...
            else
            {
                ...
                if ($package == "Pro" || $package == "Business" || $package == "" && Auth::User()->role == "admin")
                {
                    ...
                    if (!empty($search_value)):
                        $artists = Artist::where('artist_name', 'LIKE', '%' . $search_value . '%')->orderBy('created_at', 'desc')
                            ->paginate(9);
                    else:
    ...
    public function store(Request $request)
    {
        ...
        if (!empty($data['password_hash']))
        {
            ...
            if ($package == "Pro" || $package == "Business" || $package == "" && Auth::User()->role == "admin")
            {
                ...
                $artist = Artist::create(["artist_name" => $data['artist_name'], "description" => $data['description'], "image" => $data['image'], "artist_slug" => $artist_slug, ]);

                $artist_id = $artist->id;

                return Redirect::to('channel/artists')
                    ->with(array(
    ...
    public function update(Request $request)
    {
        ...
        if (!empty($data['password_hash']))
        {
            ...
            if ($package == "Pro" || $package == "Business" || $package == "" && Auth::User()->role == "admin")
            {
                ...
                $artist->update(["artist_name" => $data['artist_name'], "description" => $data['description'], "artist_slug" => $artist_slug, ]);

                if (!empty($data['image']))
                {
                    $artist->update(["image" => $data['image'], ]);
                }

                return Redirect::to('channel/artists/edit' . '/' . $id)->with(array(
    ...
    public function destroy($id)
    {
        ...
        if (!empty($data['password_hash']))
        {
            ...
            if ($package == "Pro" || $package == "Business" || $package == "" && Auth::User()->role == "admin")
            {
                ...
                return Redirect::to('admin/artists')->with(array(

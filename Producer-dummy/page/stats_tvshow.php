<?php  
	...
	$sql = "select share_percent from producers_map where producer_id=$runmawi_producer_userid and video_id=$param1 and category=2 and status=1";
	...
	$title = "";
	$type = "";
	$sql = "select id, title, type status from tvshows where status>=0 and id = $param1 ";
	$res = mysqli_query($con, $sql);
	if(mysqli_num_rows($res) > 0){
		$row = mysqli_fetch_array($res);
		$title = $row[1];
	...
	//total purchased
	$sql = "select count(*), sum(amount) from payment_webhook where amount>0 and status = 'TXN_SUCCESS' and order_id in (select order_id from orders where status=1 and payment_status=1 and (category=2 or category=21) and video_id in (select video_id from producers_map where producer_id=$runmawi_producer_userid and status>=0 and category=2 and video_id=$param1))";
	$res = mysqli_query($con, $sql);
	if(mysqli_num_rows($res) > 0){
		$row = mysqli_fetch_array($res);
	...
	//today purchased
	$sql = "select count(*), sum(amount) from payment_webhook where amount>0 and status = 'TXN_SUCCESS' and DATE(created_at)=CURDATE() and order_id in (select order_id from orders where status=1 and payment_status=1 and (category=2 or category=21) and video_id in (select video_id from producers_map where producer_id=$runmawi_producer_userid and status>=0 and video_id=$param1 and category=2))";
	$res = mysqli_query($con, $sql);
	if(mysqli_num_rows($res) > 0){
		$row = mysqli_fetch_array($res);
	...
	//this month
	$sql = "select count(*), sum(amount) from payment_webhook where amount>0 and status = 'TXN_SUCCESS' and MONTH(created_at)=MONTH(CURDATE()) and YEAR(created_at)=YEAR(CURDATE()) and order_id in (select order_id from orders where status=1 and payment_status=1 and (category=2 or category=21) and video_id in (select video_id from producers_map where producer_id=$runmawi_producer_userid and status>=0 and video_id=$param1 and category=2))";
	$res = mysqli_query($con, $sql);
	if(mysqli_num_rows($res) > 0){
		$row = mysqli_fetch_array($res);
	...
	//previous month
	$sql = "select count(*), sum(amount) from payment_webhook where amount>0 and status = 'TXN_SUCCESS' and MONTH(created_at)=MONTH(CURDATE()) - 1 and YEAR(created_at)=YEAR(CURDATE()) and order_id in (select order_id from orders where status=1 and payment_status=1 and (category=2 or category=21) and video_id in (select video_id from producers_map where producer_id=$runmawi_producer_userid and status>=0 and video_id=$param1 and category=2))";
	$res = mysqli_query($con, $sql);
	if(mysqli_num_rows($res) > 0){
		$row = mysqli_fetch_array($res);
	...
	//this year
	$sql = "select count(*), sum(amount) from payment_webhook where amount>0 and status = 'TXN_SUCCESS' and YEAR(created_at)=YEAR(CURDATE()) and order_id in (select order_id from orders where status=1 and payment_status=1 and (category=2 or category=21) and video_id in (select video_id from producers_map where producer_id=$runmawi_producer_userid and status>=0 and video_id=$param1 and category=2))";
	$res = mysqli_query($con, $sql);
	if(mysqli_num_rows($res) > 0){
		$row = mysqli_fetch_array($res);
	...
	$sql = "select count(*) from payment_webhook where amount=0 and status = 'TXN_SUCCESS' and order_id in (select order_id from orders where status=1 and payment_status=1 and (category=2 or category=21) and video_id in (select video_id from producers_map where producer_id=$runmawi_producer_userid and status>=0 and (category=1 or category=2) and video_id=$param1))";
	$res = mysqli_query($con, $sql);
	if(mysqli_num_rows($res) > 0){
		$row = mysqli_fetch_array($res);
		$promotions = $row[0];
	...
	$sql = "select count(*), sum(amount), DATE(created_at) from payment_webhook where amount>0 and status = 'TXN_SUCCESS' and order_id in (select order_id from orders where status=1  and payment_status=1 and (category=2 or category=21) and video_id in (select video_id from producers_map where producer_id=$runmawi_producer_userid and status>=0  and video_id=$param1 and (category=1 or category=2)))  group by DATE(created_at) order by DATE(created_at) ";
	$res = mysqli_query($con, $sql);
	if(mysqli_num_rows($res) > 0){

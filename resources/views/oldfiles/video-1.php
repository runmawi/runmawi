...
<?php
  ...
  <?php }  
		...
		<div id="video_container" class="fitvid">
			<video data-dashjs-player id="videojs-seek-buttons-player" autoplay onplay="playstart()" onended="autoplay1()" class="video-js vjs-default-skin" controls preload="auto" poster="<?= URL::to('/public/uploads/') . '/images/' . $video->image ?>"  data-setup='{ "playbackRates": [0.5, 1, 1.5, 2] }' width="100%" style="width:100%;" data-authenticated="<?= !Auth::guest() ?>">
      <!-- <source src="dash-stream.mpd" type="application/dash+xml"> -->
...
-->
	...
	<script type="text/javascript">
      ...
      //like dislike
      $('.like').click(function(){
        var like = $(this).attr("data-val");
        var video_id = $(this).attr('videoid');
        alert(like);
        $.post('<?php echo URL::to('like-dislike') ?>', { video_id : $(this).attr('videoid'), _token: '<?= csrf_token(); ?>',like:like }, 
                    function(data){
         });
				          //	$(this).toggleClass('active');
			});

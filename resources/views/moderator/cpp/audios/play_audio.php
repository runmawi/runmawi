@include('moderator.cpp.audios.header')
<?php 
    //include('moderator/cpp/audios/header.php'); 
...
<div id="audio_bg" >
<div id="audio_bg_dim" <?php if($audio->access == 'guest' || ($audio->access == 'subscriber' && !empty($user_id)) ): ?><?php else: ?>class="darker"<?php endif; ?>></div>
<div class="container-fluid">

<?php if($audio->access == 'guest' || ( ($audio->access == 'subscriber' || $audio->access == 'registered') && !empty($user_id) && Auth::user()->subscribed()) || (!empty($user_id) && (Auth::user()->role == 'demo' || Auth::user()->role == 'admin')) || (!empty($user_id) && $audio->access == 'registered' && $settings->free_registration && Auth::user()->role == 'registered') || (($audio->access == 'subscriber' || $audio->access == 'registered') && $ppv_status == 1)): ?>
...
<a aria-hidden="true" class="favorite <?php echo audiofavorite($audio->id);?>" data-authenticated="<?= !empty($user_id) ?>" data-audio_id="<?= $audio->id ?>"><?php if(audiofavorite($audio->id) == "active"): ?><i id="ff" class="fa fa-heart" ></i><?php else: ?><i id="ff" class="fa fa-heart-o" ></i><?php endif; ?></a>
    ...
    <th>Duration</th></tr>
      ...
      <tr class="audio-lpk">
          ...
          <td> <img src="<?= URL::to('/').'/public/uploads/images/'. $other_audio->image ?>"  class="img-responsive" / width="50"></td>
           ...
           <td>Arstist</td>
           <td><a aria-hidden="true" class="favorite <?php echo audiofavorite($other_audio->id);?>" data-authenticated="<?= !empty($user_id) ?>" data-audio_id="<?= $other_audio->id ?>"><?php if(audiofavorite($other_audio->id) == "active"): ?><i class="fa fa-heart" ></i><?php else: ?><i class="fa fa-heart-o" ></i><?php endif; ?></a></td>
...
<div class="clear"></div>
<?php if(!empty($user_id) && $audio->access == 'subscriber'): ?>

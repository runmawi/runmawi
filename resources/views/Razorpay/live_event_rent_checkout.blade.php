...
var options = {
    ...
    "modal": {
    "ondismiss": function(){
        window.location.href = "{{ URL::to('/live/'.$response['Video_slug']) }} "
        },
    }
};
...
rzp1.on('payment.failed', function (response) {
    ...
    fetch("{{ url('/RazorpayLiveRent_Paymentfailure') }}", {
        ...
        body: JSON.stringify({
            ...
            live_event_id: "{{ $response['live_event_id'] }}",
            amount: "{{ $response['amount'] }}",

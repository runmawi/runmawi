<?php include('header.php');?>
 ...
 <?php } ?>
         ...
         <?php if(isset($channelwatchlater)) :
                           foreach($channelwatchlater as $video): ?>
        <div class="modal fade bd-example-modal-xl<?= $video->id;?>"  id="vidModal"   tabindex="-1" role="dialog" aria-labelledby="myExtraLargeModalLabel" aria-hidden="true">
     ...
     <div class="modal-body playvid">
                        <?php if($video->type == 'embed'): ?>
                               ...
                               <?php  elseif($video->type == 'file'): ?>
                                   <div id="video_container" class="fitvid">
                                   <video id="videojs-seek-buttons-player"   onplay="playstart()"  class="video-js vjs-default-skin" controls preload="auto" poster="<?= URL::to('/public/') . '/uploads/images/' . $video->image ?>"  data-setup='{ "playbackRates": [0.5, 1, 1.5, 2] }' width="100%" style="width:100%;" data-authenticated="<?= !Auth::guest() ?>">
...
onclick="document.getElementById('videojs-seek-buttons-player').pause();" id="<?= $video->id;?>"  ><span aria-hidden="true">X</span></button>

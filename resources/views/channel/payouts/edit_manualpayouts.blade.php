        ...
        <div class="col-md-12">
            <div class="iq-card-header  justify-content-between">
                <div class="iq-header-title">
                    <h4 class="card-title">Channel Partners Manual Payouts :</h4>
                </div>
            </div>

            <div class="clear"></div>
                <div class="modal-body">
                    <form id="payouts_form"  accept-charset="UTF-8"  file="1"action="{{ URL::to('admin/channel/update_payouts') }}" method="post" enctype="multipart/form-data">
                    
        
                       <div class="form-group {{ $errors->has('slider') ? 'has-error' : '' }}">
                            <label> Channel Partner Name:</label>
                            <input type="text" class="form-control" name="user_name" id="user_name" value="@if(count($payouts) > 0) {{ $payouts[0]->channel_name }} @endif" readonly/>
                        </div>
                        ...
                        <input type="hidden" name="id" id="id" value="@if(count($payouts) > 0) {{ $payouts[0]->channel_id }} @endif" />

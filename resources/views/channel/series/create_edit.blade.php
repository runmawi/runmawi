...
<div id="content-page" class="content-page">
    <div class="d-flex">
    <a class="black" href="{{ URL::to('channel/series-list') }}"> Series List</a>
        <a class="black" style="background:#fafafa!important;color: #006AFF!important;" href="{{ URL::to('channel/series/create') }}"> Add New Series</a></div>
		...
		<div class="admin-section-title">
            <div class="row p-3">
                ...
                <a href="javascript:;" onclick="jQuery('#add-new').modal('show');" class="btn btn-primary"><i class="fa fa-plus-circle"></i>Create Season</a>
                    ...
                    <!-- <a href="{{ URL::to('channel/season/create/') . '/' . $series->id  }}" class="btn btn-primary"><i class="fa fa-plus-circle"></i>Create Season</a> -->
                </div>
            </div>
		<!-- Add New Modal -->
		...
		<div class="modal-dialog">
			<div class="modal-content">
				...
				<div class="modal-body">
					<form id="new-cat-form" accept-charset="UTF-8" action="{{ URL::to('channel/season/create/') }}" enctype="multipart/form-data" method="post">
						<input type="hidden" name="_token" value="<?= csrf_token() ?>" />
...
<table class="table table-bordered genres-table">
		...
		<tr class="table-header">
			...
			<tr>
				...
				<td>
					<p>
						<a href="{{ URL::to('channel/season/edit') . '/' . $series->id. '/' . $seasons_value->id }}" class="btn btn-xs btn-black"><span class="fa fa-edit"></span> Manage Episodes</a>
						<a href="{{ URL::to('channel/season/edit') . '/' . $seasons_value->id }}" class="btn btn-xs btn-black"><span class="fa fa-edit"></span> Edit Season</a>
						<a href="{{ URL::to('channel/season/delete') . '/' . $seasons_value->id }}" class="btn btn-xs btn-white delete"><span class="fa fa-trash"></span> Delete</a>
					</p>
				</td>
			</tr>
			@endforeach
...
<script>
	$(document).ready(function(){
		...
		$('#player_image').on('change', function(event) {
			...
			console.log('validation widtth ' + valid_player_Width);
    ...
    $("#duration").mask("00:00:00");
		...
		$('#titles').change(function(){
   		//  alert(($('#title').val()));
			...
			var title = $('#title').val();
			$.ajax({
				url:"{{ URL::to('channel/titlevalidation') }}",
				method:'GET',
				data: {
					_token: '{{ csrf_token() }}',
					title: $('#title').val()
